...
func TestGetLinesBetween(t *testing.T) {
	...
	for _, c := range testdata {
		...
		versionRange := sdk.FindVersionTitles(lines, 2)
		title, previous, rest := sdk.GetLinesBetween(lines, versionRange)
...
func TestFindVersionTitles(t *testing.T) {
	...
	for _, c := range testdata {
		versions := sdk.FindVersionTitles(strings.Split(c.input, "\n"), -1)
