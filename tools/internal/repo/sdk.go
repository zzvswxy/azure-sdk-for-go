...
type RepositoryWithChangelog interface {
	ReportForCommit(commit string) (sdk.RepoContent, error)
}
...
func (s *sdkRepository) ReportForCommit(commit string) (sdk.RepoContent, error) {
	if commit != "" {
	...
	return sdk.GetRepoContent(s.Root())
}
